<?php
   ...
   if(isset($_POST['keyword'])){
    // var_dump($_POST['keyword']);
     ...
     $sql="select topic.*,class.name,user.img,user.username from topic,class,user where topic.class_id=class.id and topic.user_id=user.id and title like '%{$keyword}%' order by topic.id desc";
   }elseif(isset($_GET['id'])){
     $id=$_GET['id'];
     $sql="select topic.*,class.name,user.img,user.username from topic,class,user where topic.class_id=class.id and topic.id={$id} and topic.user_id=user.id order by topic.id desc";
      $sqlViews="update topic set views=views+1 where id={$id}";
      $rstviews=$pdo->prepare($sqlViews);
      $rstviews->execute(); 
      echo "<input type='hidden' id='jmu'>";    
    }else{
       $sql="select topic.*,class.name,user.img,user.username from topic,class,user where topic.class_id=class.id and topic.user_id=user.id order by topic.id desc";
    }
    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="header-navbar">
        <ul class="nav navbar-nav navbar-right">
          <li class="hidden-index active"><a data-cont="平台首页" href="index.php">平台首页</a></li>
          <?php 
  ...
  <div class="content-wrap">
    <div class="content">
      ...
      <?php 
         foreach($rows as $row){      
       ?>
      <article class="excerpt excerpt-1"><a class="focus" <?php echo "href='index.php?id={$row['id']}'"; ?> title=""><img class="thumb" <?php echo "src='user/{$row['img']}'"; ?> alt=""><?php echo "{$row['username']}"; ?></a>
      
        <header><?php echo"<a class='cat' href='indexClass.php?classid={$row['class_id']}'>" ?><?php echo $row['name'] ?><i></i></a>
          <?php echo "<h2><a href='index.php?id={$row['id']}' title=''>{$row['title']}</a></h2>" ; ?>
        </header>
        <p class="meta">
        ...
        <p class="note"><?php $b=strlen($str);if(isset($_GET['id'])){echo $str;} elseif($b<5005){echo "<a href='index.php?id={$row['id']}' style='color:black'>".substr($str , 0 , 5005)."</a>";}else{echo "<a href='index.php?id={$row['id']}' style='color:black'>".substr($str , 0 , 5005)."</a>"."。。。<a style='float:right;margin-right:70px' href='index.php?id={$row['id']}'>点击查看更多>></a>";} ?></p>
         <a class="cat" href="javascript:" style="margin-top: 15px">用户评论：<i></i><i class="glyphicon glyphicon-comment"></i><span class="commentTimes"><?php echo "{$row['comment_times']}" ?></span></a><span class='meta user_comment' style='margin-left: 10%;font-size: 14px'><a class="comment" <?php if(!isset($_SESSION['user_name'])){?> href='#loginModal' <?php } ?> data-toggle='modal'><i class="glyphicon glyphicon-comment"></i>点击评论</a></span>
            ...
            <div class="said">
              <div></br>              
                <input class="ipt form-control" type="text" name="conten" required autofocus autocomplete="off" />
                <input type="hidden" name="topic_id" value="<?php echo $row['id'];?>"/>
                <input type="button" class="btn btn-primary" style="margin-top: 5px" value="提交评论" <?php if(!isset($_SESSION['user_name'])){ echo "disabled"; }?> /> 
...
<script type="text/javascript">
  $(function(){
    ...
    if(!(document.getElementById('jmu'))){     
